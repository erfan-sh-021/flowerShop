...
export default function CheckoutPage() {
  ...
  const [hydrated, setHydrated] = useState(false); // برای جلوگیری از redirect زودهنگام
  ...
  // انتظار برای hydrating store قبل از redirect
  useEffect(() => {
    const timer = setTimeout(() => setHydrated(true), 50); // مقدار مناسب تست شود
    return () => clearTimeout(timer);
  }, []);

  // redirect فقط وقتی store آماده شد
  useEffect(() => {
